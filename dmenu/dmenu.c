...
readxresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		if (XrmGetResource(xdb, "dmenu.color17", "*", &type, &xval))
        ...
        if (XrmGetResource(xdb, "dmenu.foreground", "*", &type, &xval))
			...
			colors[SchemeCursor][ColBg] = strdup(colors[SchemeCursor][ColBg]);
		if (XrmGetResource(xdb, "dmenu.background", "*", &type, &xval))
			colors[SchemeCursor][ColFg] = strdup(xval.addr);
		else
			colors[SchemeCursor][ColFg] = strdup(colors[SchemeCursor][ColFg]);
