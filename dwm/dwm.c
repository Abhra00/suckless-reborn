...
enum { SchemeNorm, SchemeSel, SchemeBtn, SchemeLt, SchemeTle }; /* color schemes */
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	w += horizpadbar; /* padding on both sides as mentioned */
	ret = m->ww - w;
		...
		if (text[i] == '^' && !isCode) {
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					drw_rect(drw, rx +x - 2 * sp, ry + vertpadbar / 2, rw, MIN(rh, bh - vp), 1, 0);
				} else if (text[i] == 'f') {
					x += atoi(text + ++i);
				}
			}
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - stw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeTle : SchemeNorm]);
